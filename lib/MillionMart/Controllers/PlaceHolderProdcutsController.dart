...
class PlaceholderProductsController extends GetxController{
  // VoiceSearchController(this.slug);
  var isLoading = true.obs;
  // var data = [].obs;
  var placeholderProductList = [].obs;
  var keyword = "";
  // String slug;
  // var slg =''.obs;
  ...
  Future<void> fetchProducts(var pName) async {
    print("title is $pName");
    try {
      isLoading(true);
      placeholderProductList.clear();
      var products;
      var url = Constants.baseUrl;
      final response = await http.get(Uri.parse(url + '/categoryproduct/$pName'));
      print('Check Res ${response.statusCode}');
      if(response.statusCode ==200){
        products = jsonDecode(response.body);
        print("response from controller... $products");
      }
      // var products = jsonDecode(await VoiceSearchServices.fetchProducts(pName));

      if (products != null) {
        placeholderProductList.add(products);
        // Get.log(productList.value.toString());
        print("placeholder data $products");
        isLoading(false);
      }
    } finally {
      isLoading(false);
...
//     isLoading(true);
//     productList.clear();
//     var url = Constants.updatedBseUrl;
